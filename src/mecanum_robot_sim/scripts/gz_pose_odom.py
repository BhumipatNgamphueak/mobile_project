...
class GzPoseOdom(Node):
    ...
    def __init__(self):
        ...
        # Low-pass filter state for velocity (EMA, alpha=0.4)
        self._vel_alpha = 0.4
        self._vx_f    = 0.0
        self._vy_f    = 0.0
        self._omega_f = 0.0
    ...
    def _handle(self, tf: TransformStamped):
        ...
        # Use the Gazebo message timestamp to avoid TF clock-skew under sim time.
        now   = Time.from_msg(tf.header.stamp)
        stamp = tf.header.stamp

        # Velocity by finite difference with EMA low-pass filter.
        # Guard: only update when dt is in a sane range (avoids spikes on
        # sim pause/burst or zero-dt duplicate messages).

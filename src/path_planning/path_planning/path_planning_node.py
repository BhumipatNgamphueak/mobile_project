...
class PathPlanningNode(Node):
    ...
    def __init__(self):
        ...
        self.sensor_timeout         = 0.5    # s — max age of odom/costmap data
        ...
        self.mcch_split_thresh  = 0.6    # m — MCCH fictitious-edge threshold
    ...
    def _replan(self):
        ...
        if (self._last_odom_time is not None and
                now_s - self._last_odom_time > self.sensor_timeout):
            self.get_logger().warn('Odom data stale — stopping robot')
            self._pub_stop()
            return
        if (self._last_costmap_time is not None and
                now_s - self._last_costmap_time > self.sensor_timeout):
            self.get_logger().warn('Costmap data stale — stopping robot')
            self._pub_stop()
            return
    ...
    def _compute_cmd_vel(self,
        ...
        for lx, ly in zip(opt_xs, opt_ys):
            if math.hypot(lx - rx, ly - ry) >= self.cmd_lookahead:
                tx, ty = lx, ly
                break
        ...
        # 3. Decelerate near goal: speed (m/s) tracks distance (m), capped at v_max.

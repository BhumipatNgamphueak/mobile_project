...
class ElasticBandPlanner:
    ...
    def _obstacle_force(
        ...
        if best_inside:
            ...
            if m < 1e-9:
                return infl, 0.0
            # Scale by (infl + penetration depth) so deeper nodes escape faster.
            mag = infl + best_d
    ...
    def _preroute(
        ...
        # Two passes: the first pass may displace nodes into an adjacent polygon;
        # the second pass catches those newly-colliding nodes.
        for _ in range(2):
            for poly in polygons:
                if len(poly) < 3:
                    continue
